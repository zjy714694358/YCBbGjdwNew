...
public class ShiOrShiliu {
     ...
     * @return
     */
    public static String toHexStringBl3(Integer integer){
        String str = "";
        if(integer<16){
            str = "000"+Integer.toHexString(integer);
        } else if (integer>=16&&integer<256) {
            str = "00"+Integer.toHexString(integer);
        }else if (integer>=256&&integer<4096) {
            str = "0"+Integer.toHexString(integer);
        }else{
            str = Integer.toHexString(integer);
        }
    ...
    public static String hexToFloatSi(String hexStr){
        float hexF = 0;
        String str = "" ;
        String hexHl = HexUtils.gaodiHuanHex(hexStr);
        try {
            hexF = Float.intBitsToFloat((int) HexUtils.parseLong(hexHl,16));
            str = StringUtils.siweiYouxiaoStr(hexF+"");
        } catch (HexUtils.NumberFormatException e) {
            e.printStackTrace();
        }
        return str;
    }
